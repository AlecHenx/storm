...
public class StringMetadataCache implements LruMap.CacheEvictionCallback<String, StringMetadata>,
    ...
    private RocksDbTrajectoryWriter dbWriter;
     ...
     */
    private StringMetadataCache(RocksDbTrajectoryWriter dbWriter, int capacity) {
     ...
     * will be ignored unless preceded by {@link #init(RocksDbTrajectoryWriter, int)}.
     *
     ...
     */
    static void init(RocksDbTrajectoryWriter dbWriter, int capacity) {
        if (instance == null) {
     ...
     *
     * @throws TrajStoreException if the cache instance was not created
     */
    static WritableStringMetadataCache getWritableStringMetadataCache() throws TrajStoreException {
        if (instance != null) {
            return instance;
        } else {
            throw new TrajStoreException("StringMetadataCache was not initialized");
        }
    }
     ...
     * @throws TrajStoreException if the cache instance was not created
     */
    static ReadOnlyStringMetadataCache getReadOnlyStringMetadataCache() throws TrajStoreException {
        if (instance != null) {
            return instance;
        } else {
            throw new TrajStoreException("StringMetadataCache was not initialized");
    ...
    // TODO: does this function need to be public?
     ...
     * @throws TrajStoreException when evicted data fails to save to the database or when the database is shutdown
     */
    @Override
    public void put(String s, StringMetadata stringMetadata, boolean newEntry) throws TrajStoreException {
        if (dbWriter.isShutdown()) {
            // another thread could be writing out the metadata cache to the database.
            throw new TrajStoreException("Shutting down");
        }
        try {
            if (newEntry) {
            ...
            throw new TrajStoreException("Failed to save string in metadata cache", e);
        }
    }
    ...
    private void writeMetadataToDisk(String key, StringMetadata val) {
        ...
        RocksDbValue rval = new RocksDbValue(key);
